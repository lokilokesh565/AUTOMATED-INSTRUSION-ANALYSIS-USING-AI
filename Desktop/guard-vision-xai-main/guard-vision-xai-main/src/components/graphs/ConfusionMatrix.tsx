import { confusionMatrixData } from "@/data/modelResults";
import type { CMData } from "@/data/analyzeData";

interface Props { cm?: CMData }

const ConfusionMatrix = ({ cm }: Props) => {
  const model = "Hybrid Voting";
  const data = cm ?? confusionMatrixData[model];
