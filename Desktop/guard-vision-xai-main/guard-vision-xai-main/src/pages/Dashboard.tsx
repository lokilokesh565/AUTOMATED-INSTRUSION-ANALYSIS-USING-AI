    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 py-8">
          ...
          <div className="card-cyber p-5 flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-2xl font-bold text-foreground">
                {uploadedFileName ? `${totalRows.toLocaleString()} rows` : "CIC-IDS2017"}
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="text-center mb-10">
          <button onClick={handleScan} disabled={scanning} className="scan-button text-lg">
            <Radar className="w-5 h-5 inline-block mr-2" />
            {scanning ? "Scanning..." : showResults ? "Re-Scan Network Traffic" : "Scan Network Traffic"}
          </button>
          <p className="text-sm text-muted-foreground mt-3">
            {uploadedData
              ? `Scan ${uploadedData.length} records from uploaded dataset`
              : "Upload a dataset above or click to scan with default test data"}
          </p>
        </div>
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <div className="animate-fade-in" style={{ animationDelay: "300ms" }}>
                <ConfusionMatrix cm={analysisResult?.confusionMatrix} />
              </div>
